...
anonymous 사용자로 인증을 시키고 인가를 해주지 않습니다.

직접 401 인증에러를 확인하려면 http header에 인증되지 않은 토큰을 넣어 확인할 수 있습니다.

헤더에 인증된 bearer 토큰을 넣어 
/api에 http request를 보내면 쿠버네티스 api 목록을 확인할 수 있습니다.

해당 클러스터의 pod 목록을 출력하기위해 아래와 같은 request를 보내면
default에 대한 인가가 되지 않아 오류가 발생하는걸 확인할 수 있습니다.
...
roleRef:
  ...
  name: my-cluster-role
```
my-service-account-token-jsw66